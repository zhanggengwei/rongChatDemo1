...
+(void)showSaveBtn:(UILongPressGestureRecognizer * )Recognizer{
    ...
    btn.frame = CGRectMake(OBJC_SCREEN_WIDTH - 80,CGRectGetMaxY(Recognizer.view.frame) + 10,70, 30);
...
+ (UIImage *)imageCompressLargeImageToFitScreen: (UIImage *)image
{
    ...
    if (imageWidth <= OBJC_SCREEN_WIDTH || imageHeight <= OBJC_SCREEN_HEIGHT)
    {
        return image;
    }
    
    newImage = [self imageByScalingWithSimple:image forSize:CGSizeMake(OBJC_SCREEN_WIDTH, OBJC_SCREEN_HEIGHT)];
...
+ (UIImage *)imageCompressLargeImageToAspectFillScreen: (UIImage *)image
{
    ...
    newImage = [self imageByScalingWithSimple:image aspectFillForSize:CGSizeMake(OBJC_SCREEN_WIDTH, OBJC_SCREEN_HEIGHT)];
